...
public class InmuebleFacadeImp
   ...
   * 
   * var unidadDeNegocioEntity =
   * unidadDeNegocioServiceImpl.findById(inmuebleCreateDto.getIdUnidadDeNegocio())
   * ;
   ...
   * 
   * @Override
   * public InmuebleDto update(Long id, InmuebleCreateDto inmuebleCreateDto)
   * throws Exception {
   * var inmuebleEntity = baseService.findById(id);
   * baseMapper.toUpdate(inmuebleEntity, inmuebleCreateDto);
   * 
   * //UNIDAD DE NEGOCIO
   * if (!inmuebleEntity.getIdUnidadDeNegocio().getId().equals(inmuebleCreateDto.
   * getIdUnidadDeNegocio())) {
   * var unidadDeNegocioEntity =
   * unidadDeNegocioServiceImpl.findById(inmuebleCreateDto.getIdUnidadDeNegocio())
   * ;
   * inmuebleEntity.setUnidadDeNegocio(unidadDeNegocioEntity);
